...
function chooseService() {
  let service = select("service").from(
    "Banker",
...
function systemConfirmation() {
  ...
  let hourText;
  if (hour.charAt(1) === "9") {
    hourText = hour.slice(1, hour.length) + " AM";
  } else {
    hourText = hour;
  }

  session.assertText(COMPONENTS.SYSTEM_CONFIRM.conclusion_time, hourText, [
    TextAssertions.modifiers.Contains,
  ]);

  session.assertText(
    COMPONENTS.SYSTEM_CONFIRM.conclusion_email,
    CUSTOMER_DETAILS.email
  );
